...
{
    PCP_DEBUG_ASSERT(sampling.size() <= m_points->size());
...
void KdTree::build_rec(int node_id, int start, int end, int level)
{
    ...
    }
    {
        ...
        if(midId-start <= m_min_cell_size || level >= PCP_KDTREE_MAX_DEPTH)
        {
            child.leaf = 1;
            child.start = start;
    ...
    }
    {
        ...
        KdTreeNode& child = nodes[childId];
        if(end-midId <= m_min_cell_size || level >= PCP_KDTREE_MAX_DEPTH)
        {
            child.leaf = 1;
            child.start = midId;
