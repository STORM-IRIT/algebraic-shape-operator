...
{
    PCP_DEBUG_ASSERT(vec.size() == to_erase.size());
...
{
    PCP_DEBUG_ASSERT(vec.size() == to_keep.size());
...
{
    PCP_DEBUG_ASSERT(to_keep.size() <= vec.size());
    PCP_DEBUG_ASSERT(std::is_sorted(to_keep.begin(), to_keep.end()));
